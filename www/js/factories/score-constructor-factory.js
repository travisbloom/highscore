...
angular.module("highScoreApp").factory("highScoreFactory", function (userDataFactory, thirdPartyFactory) {
  //captures all the constructed HighScoreObj's
  var highScoreArray = undefined;
  /**
  ...
  function prepareScoreProperties(properties) {
    ...
    if (!isNaN(properties.currentScore)) {
      ...
      if (properties.currentScore >= (this.highScore || properties.currentScore)) properties.highScore = properties.currentScore;
    }
    return properties;
  }
  /**
  ...
  var HighScoreObj = (function () {
    function HighScoreObj(obj) {
      _classCallCheck(this, HighScoreObj);

      Object.assign(this, obj);
    }

    _createClass(HighScoreObj, {
      saveObj: {
        /**
         * save the updated object attributes
         ***/

        value: function saveObj(obj) {
          var userData = userDataFactory.data;
          //index of object
          var index = userData.scores.map(function (score) {
            return score.id;
          }).indexOf(this.id);
      ...
      pullScore: {
        ...
        value: function pullScore() {
          var _this = this;

          var metaData = this.metaData && this.metaData.queryParams ? this.metaData.queryParams : undefined;
  ...
  ***/
   ...
   ***/
  return {
    newScore: function newScore(obj) {
      var userData = userDataFactory.data;
    ...
    reorderScores: function reorderScores(fromIndex, toIndex) {
      //save new story data
      var userData = userDataFactory.data;
